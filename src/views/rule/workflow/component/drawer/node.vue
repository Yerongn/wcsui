...
// 获取父组件数据
const getParentData = async (data: object) => {
	state.tabsActive = '1';
	state.node = data;
	if (state.node.id === '14' || state.node.id === '21') {
		...
		if (state.node.id === '14')
			state.node.from[0]['options'] = state.processList.map((item: Process) => ({ value: item.processType, label: item.processDescribe }));
		else {
			state.node.from[0]['options'] = state.processDataTypeList.map((item: ProcessDataType) => ({
				value: item.processDataType,
