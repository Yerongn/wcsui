<template>
	<div class="system-menu-dialog-container">
		<el-dialog :title="state.dialog.title" v-model="state.dialog.isShowDialog" width="769px">
			<el-form ref="menuDialogFormRef" :model="state.form" size="default" label-width="80px">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="菜单名称" prop="title" :rules="[{ required: true, message: '菜单名称不能为空', trigger: 'blur' }]">
							<el-input v-model="state.form.title" placeholder="请输入菜单名称" clearable></el-input>
						</el-form-item>
					</el-col>
					<template v-if="state.form.type != 'Component'">
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
							<el-form-item label="路由名称" prop="name" :rules="[{ required: true, message: '路由名称不能为空', trigger: 'blur' }]">
								<el-input v-model="state.form.name" placeholder="路由名称" clearable />
							</el-form-item>
						</el-col>
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="权限标识">
							<el-input v-model="state.form.permission" placeholder="请输入权限标识" clearable></el-input>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
...
const openDialog = (type: string, row?: any) => {
	if (type === 'edit') {
		...
		// state.form = JSON.parse(JSON.stringify(menu));
...
const onSubmit = () => {
	...
	menuDialogFormRef.value.validate(async (valid: boolean) => {
		...
		if (state.dialog.type === 'add') {
			await useMenuApi().addMenu(state.form);
		} else {
			await useMenuApi().updateMenu(state.form);
		}
		closeDialog(); // 关闭弹窗
		emit('refresh');
		// if (state.dialog.type === 'add') { }
